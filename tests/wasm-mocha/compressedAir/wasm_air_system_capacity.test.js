...
describe('Compressed Air Air System Capacity', function () {
    ...
    it('should calculate total pipe volume, receiver volume, and system capacity', function () {
        // Build receiver capacities vector (gallons converted to cubic feet internally by the module)
        const receiverCapacitiesGal = [400, 500, 660, 1060];
        const receiverCapacities = new moduleInstance.DoubleVector();
        for (const capacity of receiverCapacitiesGal) {
            receiverCapacities.push_back(capacity);
        }

        // Pipe lengths in feet for each standard nominal pipe size
        const pipeLengths = {
            oneHalf: 3000,
            threeFourths: 2000,
            one: 1000,
        ...
        const pipeData = new moduleInstance.PipeData(
        ...
        const airSystemCapacity = new moduleInstance.AirSystemCapacity(pipeData, receiverCapacities);
        const result = airSystemCapacity.calculate();

        // Collect receiver capacities from the result vector
        const resultReceiverCapacities = [];
