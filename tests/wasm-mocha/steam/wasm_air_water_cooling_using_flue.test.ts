...
import createModule, { type AirWaterCoolingUsingFlueOutput, type MeasurToolsSuite } from 'measur-tools-suite';
...
describe('Steam air water cooling using flue', function () {
    ...
    it('should calculate airWaterCoolingUsingFlue results correctly', function () {
        const gasInstance = new moduleInstance.GasCompositions('Gas', 94.1, 3.02, 1.41, 0.01, 0.42, 0.28, 0.0, 0.0, 0.7, 0, 0.01);
        try {
            const results: AirWaterCoolingUsingFlueOutput = moduleInstance.airWaterCoolingUsingFlue(gasInstance, 116, 300, 125, 70, 60, 0.04, 60, 0);
