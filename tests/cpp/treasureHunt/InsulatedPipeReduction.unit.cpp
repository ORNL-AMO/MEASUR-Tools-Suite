...
TEST_CASE("Rayleigh Number", "[InsulationServices][util]") {
    ...
    double result             = InsulatedPipeReduction::calculateRayleighNumber(
        expCoefficient, surfaceTemperature, ambientTemperature, diameter, kinematicViscosity, alpha);
...
TEST_CASE("Radiative Heat Transfer Coefficient", "[InsulationServices][util]") {
    ...
    double surfaceTemperature = 300.817;
    double ambientTemperature = 299.817;
    double result = InsulatedPipeReduction::calculateRadiativeHeatTransferCoefficient(emissivity, surfaceTemperature,
                                                                                      ambientTemperature);
...
TEST_CASE("Convective Heat Transfer Coefficient", "[InsulationServices][util]") {
    ...
    double diameter        = 0.13335;
    double result =
        InsulatedPipeReduction::calculateConvectiveHeatTransferCoefficient(nusselt, airConductivity, diameter);
...
TEST_CASE("Insulated Pipe", "[InsulatedPipeReduction][util]") {
    ...
    InsulatedPipeInput input(operatingHours, pipeLength, pipeDiameter, pipeThickness, pipeTemperature,
                             ambientTemperature, windVelocity, systemEfficiency, insulationThickness, pipeEmissivity,
                             jacketEmissivity, pipeMaterialCoefficients, insulationMaterialCoefficients);
...
TEST_CASE("Non-Insulated Pipe", "[InsulatedPipeReduction][util]") {
    ...
    InsulatedPipeInput input(operatingHours, pipeLength, diameter, pipeThickness, pipeTemperature, ambientTemperature,
                             windVelocity, systemEfficiency, insulationThickness, pipeEmissivity, jacketEmissivity,
                             pipeMaterialCoefficients, insulationMaterialCoefficients);
