...
TEST_CASE( "SQLite - getSolidLoadChargeMaterials", "[databases]" ) {
...
TEST_CASE( "SQLite - update all materials", "[databases]" ) {
    auto sqlite = SQLite(":memory:", true);
    ...
    // {
    //     auto motorData = databases.getMotorData();
    ...
    //     databases.insertMotorData(motor1);
    //     databases.insertMotorData(motor2);

	//     auto custom = databases.getMotorData().at(0);
    //     auto custom2 = databases.getMotorData().at(1);


    //     custom.setHp(5);
    //     custom2.setHp(6);

	//     databases.updateMotorData(custom);
    //     databases.updateMotorData(custom2);

    //     CHECK(databases.getMotorDataById(0).getHp() == 5);
    //     CHECK(databases.getMotorDataById(1).getHp() == 6);
    // }
...
TEST_CASE( "SQLite - deleteMaterials", "[databases]" ) {
	auto sqlite = SQLite(":memory:", true);
...
//TEST_CASE( "SQLite - test db init", "[databases]" ) {
//    {
//        std::ifstream ifs("measur_tools_suite.db");
//        auto const isOpen = ifs.is_open();
//        ifs.close();
//        auto databases = SQLite("measur_tools_suite.db", ! isOpen);
//
//        auto res = databases.getSolidLoadChargeMaterials();
...
//        databases.insertSolidLoadChargeMaterials(expected);
//
//        res = databases.getSolidLoadChargeMaterials();
...
TEST_CASE( "SQLite - TestSolidLoadChargeMaterialsMigrations", "[databases]" ) {
...
TEST_CASE( "SQLite - getGasLoadChargeMaterials", "[databases]" ) {
    auto sqlite = SQLite(":memory:", true);
...
TEST_CASE( "SQLite - CustomGasLoadChargeMaterials", "[databases]" ) {
    auto sqlite = SQLite(":memory:", true);
...
TEST_CASE( "SQLite - getLiquidLoadChargeMaterials", "[databases]" ) {
    auto sqlite = SQLite(":memory:", true);
...
TEST_CASE( "SQLite - customLiquidLoadChargeMaterials", "[databases]" ) {
    auto sqlite = SQLite(":memory:", true);
...
TEST_CASE( "SQLite - getGasFlueGasMaterials", "[databases]" ) {
    auto sqlite = SQLite(":memory:", true);
...
TEST_CASE( "SQLite - CustomGasFlueGasMaterials", "[databases]" ) {
    auto sqlite = SQLite(":memory:", true);
...
TEST_CASE( "SQLite - getSolidLiquidFlueGasMaterials", "[databases]" ) {
    auto sqlite = SQLite(":memory:", true);
...
TEST_CASE( "SQLite - CustomSolidLiquidFlueGasMaterials", "[databases]" ) {
    auto sqlite = SQLite(":memory:", true);
...
TEST_CASE( "SQLite - getAtmosphereSpecificHeat", "[databases]" ) {
    auto sqlite = SQLite(":memory:", true);
...
TEST_CASE( "SQLite - CustomAtmosphereSpecificHeat", "[databases]" ) {
    auto sqlite = SQLite(":memory:", true);
...
TEST_CASE( "SQLite - getWallLossesSurface", "[databases]" ) {
    auto sqlite = SQLite(":memory:", true);
...
TEST_CASE( "SQLite - CustomWallLossesSurface", "[databases]" ) {
    auto sqlite = SQLite(":memory:", true);
...
TEST_CASE( "SQLite - Motor Data inserts and updates and selects", "[databases][motor]" ) {
...

    {
        ...
        databases.insertMotorData(motorToInsert);
        auto const motors = databases.getMotorData();
	    compare(motorToInsert, motors.back());

        auto const customMotors = databases.getCustomMotorData();
        compare(motorToInsert, customMotors[0]);

        databases.deleteMotorData(motorToInsert.getId());
	    //CHECK(databases.getCustomMotorData().size() == 0);
    }
    */
}

TEST_CASE( "SQLite - Calculate nominal efficiency from data", "[databases][motor]" ) {
    ...
    //motor = databases.getMotorDataById(1351);
    //CHECK(motor.getNominalEfficiency() == Approx(89.9286895162));
}

TEST_CASE( "SQLite - Pump Data inserts and updates and selects", "[databases][pump]" ) {
    ...
    auto sqlite = SQLite(":memory:", true);
    //auto databases = SQLite("cpp_measur_tools_suite.db", true);
    //auto databases = SQLite("test.db", true);

    {
        ...
        //databases.deletePumpData(2);
        //compare(databases.getPumpData().back(), expected);
    }
}
