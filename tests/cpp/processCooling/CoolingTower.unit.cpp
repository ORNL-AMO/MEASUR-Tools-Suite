...
TEST_CASE("Cooling Tower Makeup Water Calculator", "[CoolingTower][Chillers]") {
    auto const test = [](const std::tuple<double, double, int, double, int, double, int, double>& input,
        ...
        CoolingTowerOperatingConditionsData operatingConditionsData(flowRate, coolingLoad, operationalHours,
                                                                    lossCorrectionFactor);
        CoolingTowerWaterConservationData   waterConservationBaselineData(baselineCyclesOfConcentration,
                                                                          baselineDriftLossFactor);
        CoolingTowerWaterConservationData   waterConservationModificationData(modificationCyclesOfConcentration,
                                                                              modificationDriftLossFactor);

        CoolingTowerMakeupWaterCalculator coolingTowerMakeupWaterCalculator(
            operatingConditionsData, waterConservationBaselineData, waterConservationModificationData);
    ...
    input1 =
        std::make_tuple(flowRate, coolingLoad, operationalHours, lossCorrectionFactor, baselineCyclesOfConcentration,
...
TEST_CASE("Cooling Tower Fan Energy Consumption Calculator", "[CoolingTower][Chillers]") {
    auto res =
        CoolingTower::FanEnergyConsumption(59.5119, 81.6495, 87.98386, 76, 1, CoolingTower::FanControlSpeedType::One,
                                           CoolingTower::FanControlSpeedType::Two);
    ...
    res = CoolingTower::FanEnergyConsumption(59.5119, 81.6495, 87.98386, 76, 1, CoolingTower::FanControlSpeedType::One,
                                             CoolingTower::FanControlSpeedType::Variable);
    CHECK(res.baselinePower == Approx(55.15));
    CHECK(res.baselineEnergy == Approx(41.142));
