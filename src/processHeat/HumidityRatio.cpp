...
double HumidityRatio::getSaturationPressure(double temperature) {
    ...
    if (tempInKelvins < 273.15) {
        pressure = exp(C1 / tempInKelvins + C2 + tempInKelvins * C3 +
                       tempInKelvins * tempInKelvins * (C4 + tempInKelvins * (C5 + C6 * tempInKelvins)) +
                       C7 * log(tempInKelvins));
    }
    else {
        pressure = exp(C8 / tempInKelvins + C9 + tempInKelvins * (C10 + tempInKelvins * (C11 + tempInKelvins * C12)) +
                       C13 * log(tempInKelvins));
    }
...
double HumidityRatio::getHumidityRatioUsingWBT() {
    ...
    this->humidityRatioUsingWBT_ =
        ((vaporizationHeat - (waterSpecificHeat - vaporSpecificHeat) * (this->wetBulbTemp_)) * ratioWB -
         airSpecificHeat * ((this->dryBulbTemp_) - (this->wetBulbTemp_))) /
        (vaporizationHeat + vaporSpecificHeat * (this->dryBulbTemp_) - waterSpecificHeat * (this->wetBulbTemp_));
