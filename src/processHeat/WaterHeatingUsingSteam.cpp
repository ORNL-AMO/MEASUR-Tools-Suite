...
WaterHeatingUsingSteam::Output WaterHeatingUsingSteam::calculate(
    const double pressureSteamIn, const double flowSteamRate, const double temperatureWaterIn,
    const double pressureWaterOut, const double flowWaterRate, const double tempMakeupWater,
    const double presMakeupWater, const double effWaterHeater, const double effBoiler, const double operatingHours) {
    ...
    const double enthalpyMakeUpWater =
        SteamProperties(presMakeupWater, SteamProperties::ThermodynamicQuantity::TEMPERATURE, tempMakeupWater)
            .calculate()
            .specificEnthalpy;

    double tempWaterOut =
        temperatureWaterIn + ((enthalpySteamIn - enthalpySteamOut) * flowSteamRate) / (4.1796 * 1000 * flowWaterRate);
    ...
    return Output(tempWaterOut, bpTempWaterOut, bpTempWarningFlag, flowByPassSteam, enthalpySteamIn, enthalpySteamOut,
                  enthalpyMakeUpWater, heatGainRate * operatingHours / effWaterHeater,
                  (flowSteamRate - flowByPassSteam) * (enthalpySteamOut - enthalpyMakeUpWater) * operatingHours /
                      effBoiler,
