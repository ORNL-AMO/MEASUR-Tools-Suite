...
double EfficiencyImprovement::getCurrentExcessAir() {
    this->currentExcessAir_ =
        (100 * 8.52381 * ((this->currentFlueGasOxygen_) / 100)) / (2 - 9.52381 * ((this->currentFlueGasOxygen_) / 100));
    return this->currentExcessAir_;
}

double EfficiencyImprovement::getNewExcessAir() {
    this->newExcessAir_ =
        (100 * 8.52381 * ((this->newFlueGasOxygen_) / 100)) / (2 - 9.52381 * ((this->newFlueGasOxygen_) / 100));
...
double EfficiencyImprovement::getCurrentAirCorrection() {
    ...
    this->currentAirCorrection_ =
        -(-1.07891327 + currentAirSpecificHeat * (this->currentFlueGasTemp_)) * (currentExcessAir / 100);
...
double EfficiencyImprovement::getCurrentCombustionAirCorrection() {
    double currentAirSpecificHeat = getCurrentAirSpecificHeat();
    double currentExcessAir       = getCurrentExcessAir();
    this->currentCombustionAirCorrection_ =
        (-1.078913827 + currentAirSpecificHeat * (this->currentCombustionAirTemp_)) * (1 + (currentExcessAir / 100));
...
double EfficiencyImprovement::getNewCombustionAirCorrection() {
    double newAirSpecificHeat = getNewAirSpecificHeat();
    double newExcessAir       = getNewExcessAir();
    this->newCombustionAirCorrection_ =
        (-1.078913827 + newAirSpecificHeat * (this->newCombustionAirTemp_)) * (1 + (newExcessAir / 100));
