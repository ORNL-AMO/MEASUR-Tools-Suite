...
double LeakageLosses::getExfiltratedGasesHeatContent() {
    ...
    // TODO find out if the documentation means tFurnace (in the code here) or the input parameter T_furnace
    // (leakageGasTemperature here)
    ...
    const double cpm =
        0.2371 - (9 * pow(10, -6) * calc) + (7 * pow(10, -8) * pow(calc, 2)) - ((3 * pow(10, -11)) * pow(calc, 3));
