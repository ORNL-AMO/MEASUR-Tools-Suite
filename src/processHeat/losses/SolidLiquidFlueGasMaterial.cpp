...
double SolidLiquidFlueGasMaterial::calculateExcessAirFromFlueGasO2(const double flueGasO2, const double carbon,
                                                                   const double hydrogen, const double sulphur,
                                                                   const double inertAsh, const double o2,
                                                                   const double moisture, const double nitrogen,
...
double SolidLiquidFlueGasMaterial::getHeatLoss() {
    ...
    const double msair           = o2sair + n2sair;
    const double mCombustionAir  = msair * (1 + excessAir);
    const double hCombustionAir =
        mCombustionAir * cpCombustionAir * (combustionAirTemperature - ambientAirTempF) / 0.075;
    ...
    const double pH2O =
        (mH2O / 0.047636) / (mCO2 / 0.116367 + mH2O / 0.047636 + mN2 / 0.074077 + mO2 / 0.084611 + mSO2 / 0.169381);
    ...
    const double hCO2 =
        carbonBar * (44.0 / 12) * (cpCO2(flueGasTemperature + 460) / 44.01) * (flueGasTemperature - ambientAirTempF);
    const double hH2O =
        (hydrogenBar * 9 + moistureBar) *
    ...
    const double hAsh =
        (inertAsh / percentTotalFuelComponents) * 0.25 * (1.8 * ashDischargeTemperature + 32 - ambientAirTempF);
