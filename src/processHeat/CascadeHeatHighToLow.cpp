...
CascadeHeatHighToLow::Output CascadeHeatHighToLow::calculate() {
    const double priFlueVolume =
        priFiringRate * stoichAirVolume * (1 + priExcessAir) * 1000000 / fuelHV + priFiringRate * 1000000 / fuelHV;
    const double hxEnergyRate =
        priFlueVolume * priFlueSpecHeat * priFlueDensity * (priExhaustTemperature - secExhaustTemperature) / 1000000;
