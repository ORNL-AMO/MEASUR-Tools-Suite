...
void O2Enrichment::calculate() {
    ...
    combustionAirCorrectionEnriched =
        (-1.078913827 + enrichedAirSpecificHeat * combAirTempEnriched) * (1 + excessAirEnriched);
    ...
    availableHeat =
        stdAvailableHeat + 100 * (9.38 * (o2CombAir - 0.21) / o2CombAir * 0.02 * ((flueGasTemp - 60) / 980));
    availableHeatEnriched   = stdAvailableHeatEnriched + 100 * (9.38 * (o2CombAirEnriched - 0.21) / o2CombAirEnriched *
                                                              0.02 * ((flueGasTempEnriched - 60) / 980));
