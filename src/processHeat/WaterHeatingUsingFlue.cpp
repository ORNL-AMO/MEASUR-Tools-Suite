...
WaterHeatingUsingFlue::calculate(gas_composition::GasComposition gasCompositions, const double tempFlueGasF,
    ...
    const double enthalpySteam =
        condSteam == SteamCondition::Saturated
            ...
            : SteamProperties(prSteam, SteamProperties::ThermodynamicQuantity::TEMPERATURE, tempSteamK)
                  .calculate()
                  .specificEnthalpy;
    const double enthalpyFW     = SaturatedProperties(prSteam, tempFWK).calculate().liquidSpecificEnthalpy;
    const double specificHeatFW = +0.000000000000326658 * pow(tempFWK, 6) - 0.000000000838033377 * pow(tempFWK, 5) +
                                  0.000000883910920876 * pow(tempFWK, 4) - 0.000490348083986228 * pow(tempFWK, 3) +
                                  0.150847712358072 * pow(tempFWK, 2) - 24.3962399473808 * tempFWK + 1624.65179353148;
    ...
    return Output(flowFlueGas, effBoiler, enthalpySteam, enthalpyFW, flowSteam, flowFW, specificHeatFG, heatCapacityFG,
                  specificHeatFW, heatCapacityFW, heatCapacityMin, ratingHeatRecFW, tempFlueGasOut, tempFWOut,
