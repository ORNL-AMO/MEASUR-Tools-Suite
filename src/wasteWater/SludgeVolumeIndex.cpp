...
SludgeVolumeIndex::Output SludgeVolumeIndex::calculate() {
    ...
    double overflowLineY2 = 1.25 * sviParamGamma * sludgeSettlingVelocity * exp(-1 * sviParamDelta * sviValue - 1) /
                            (sviParamAlpha + sviParamBeta * sviValue);
    ...
    for (int i = 0; i < dataPointsCount; i++) {
        ...
        graphData[i].SolidsFlux = Round(graphData[i].SolidsConcentration * sviParamGamma * sludgeSettlingVelocity *
                                        exp(-1 * sviParamDelta * sviValue - (sviParamAlpha + sviParamBeta * sviValue) *
                                                                                graphData[i].SolidsConcentration));
    }

    return Output(totalClarifierArea, surfaceOverflow, appliedSolidsLoading, rasConcentration, rasConcentration * 1000,
                  appliedSolidsLoading, overflowLineY2 * totalClarifierArea * 1000 / influentFlow, overflowLineY2,
