...
std::ostream& operator<<(std::ostream& stream, const FlashTank& flashTank) {
    stream << "FlashTank[" << "inletWaterPressure=" << flashTank.inletWaterPressure
           << ", inletWaterMassFlow=" << flashTank.inletWaterMassFlow << ", tankPressure=" << flashTank.tankPressure
...
void FlashTank::calculateProperties() {
    ...
    auto saturatedProperties =
        SaturatedProperties(tankPressure, SaturatedTemperature(tankPressure).calculate()).calculate();
    double liquidMassFlow;
    if (inletWaterProperties.specificEnthalpy > saturatedProperties.gasSpecificEnthalpy) {
        liquidMassFlow = 0;
        // TODO question density is 0 below bc saturated properties doesn't return density, same with both sp objects
        // here
        sp                              = {saturatedProperties.temperature,
                                           saturatedProperties.pressure,
                                           0,
                                           saturatedProperties.liquidSpecificVolume,
                                           0,
                                           saturatedProperties.liquidSpecificEnthalpy,
                                           saturatedProperties.liquidSpecificEntropy};
        outletLiquidSaturatedProperties = {liquidMassFlow, liquidMassFlow * saturatedProperties.liquidSpecificEnthalpy,
                                           sp};
    }
        ...
        liquidMassFlow = inletWaterMassFlow *
                         (inletWaterProperties.specificEnthalpy - saturatedProperties.gasSpecificEnthalpy) /
    ...
    sp = {saturatedProperties.temperature,
          saturatedProperties.pressure,
          1,
          saturatedProperties.gasSpecificVolume,
          0,
          saturatedProperties.gasSpecificEnthalpy,
          saturatedProperties.gasSpecificEntropy};
