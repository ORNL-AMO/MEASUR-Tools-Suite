...
PrvWithDesuperheating PrvCalculator::calcHighToMediumPrvWithDesuperheating(
    ...
    const Boiler& boiler, const SteamSystemModelerTool::FluidProperties& highPressureHeaderOutput,
    const std::shared_ptr<HeaderNotHighestPressure>& mediumPressureHeaderInput) const {
    const double prvMassFlow = calcPrvMassFlow(
...
double PrvCalculator::getTurbineMassFlow(const std::shared_ptr<Turbine>& turbine,
                                         const std::string&              turbineName) const {
    std::string turbineName_ = turbineName;
    turbineName_             = turbineName_; // keep this or fix unused variable
    double massFlow          = 0;
