...
std::ostream& operator<<(std::ostream& stream, const Deaerator& deaerator) {
    stream << "Deaerator[" << "deaeratorPressure=" << deaerator.deaeratorPressure << ", ventRate=" << deaerator.ventRate
           << ", feedwaterMassFlow=" << deaerator.feedwaterMassFlow << ", waterPressure=" << deaerator.waterPressure
           << ", waterQuantityValue=" << deaerator.waterQuantityValue
           << ", waterQuantityType=" << static_cast<int>(deaerator.waterQuantityType)
           << ", steamPressure=" << deaerator.steamPressure << ", steamQuantityValue=" << deaerator.steamQuantityValue
...
void Deaerator::calculateProperties() {
    auto const sp =
        SaturatedProperties(deaeratorPressure, SaturatedTemperature(deaeratorPressure).calculate()).calculate();
    SteamSystemModelerTool::SteamPropertiesOutput steamProps = {sp.temperature,
                                                                sp.pressure,
                                                                0,
                                                                sp.liquidSpecificVolume,
                                                                1 / sp.liquidSpecificVolume,
                                                                sp.liquidSpecificEnthalpy,
                                                                sp.liquidSpecificEntropy};
    feedwaterProperties = {feedwaterMassFlow, steamProps.specificEnthalpy * feedwaterMassFlow, steamProps};

    steamProps = {sp.temperature,         sp.pressure,          1, sp.gasSpecificVolume, 1 / sp.gasSpecificVolume,
                  sp.gasSpecificEnthalpy, sp.gasSpecificEntropy};
    auto const ventedSteamMassFlow = (ventRate / 100) * feedwaterMassFlow;
    ventedSteamProperties = {ventedSteamMassFlow, steamProps.specificEnthalpy * ventedSteamMassFlow, steamProps};
    ...
    auto const inletSteamMassFlow =
        neededEnergyFlow / (inletSteamProps.specificEnthalpy - inletWaterProps.specificEnthalpy);
    ...
    inletWaterProperties = {inletWaterMassFlow, inletWaterEnergyFlow, inletWaterProps};

    inletSteamProperties = {inletSteamMassFlow, inletSteamEnergyFlow, inletSteamProps};
}
