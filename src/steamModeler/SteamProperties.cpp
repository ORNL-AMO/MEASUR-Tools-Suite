...
SteamSystemModelerTool::SteamPropertiesOutput SteamProperties::waterPropertiesPressureTemperature(const double p,
                                                                                                  const double t) {
...
SteamSystemModelerTool::SteamPropertiesOutput SteamProperties::waterPropertiesPressureEnthalpy(const double pressure,
    ...
    if ((pressure < SteamSystemModelerTool::PRESSURE_CRIT) && (enthalpy >= pressureSatProps.liquidSpecificEnthalpy) &&
        (enthalpy <= pressureSatProps.gasSpecificEnthalpy)) {
        const double quality = (enthalpy - pressureSatProps.liquidSpecificEnthalpy) /
                               (pressureSatProps.gasSpecificEnthalpy - pressureSatProps.liquidSpecificEnthalpy);
        const double specificVolume =
            (pressureSatProps.gasSpecificVolume - pressureSatProps.liquidSpecificVolume) * quality +
            pressureSatProps.liquidSpecificVolume;
        return {
            pressureSatProps.temperature,
            pressure,
            quality,
            specificVolume,
            1 / specificVolume,
            /* TODO Density question was commented out ->   1 / (reducedPressure * gibbsPi * t * r / p / 1000.0)},
               //density in kg/m³ */
            enthalpy,
            (pressureSatProps.gasSpecificEntropy - pressureSatProps.liquidSpecificEntropy) * quality +
                pressureSatProps.liquidSpecificEntropy,
        };
    }
...
SteamSystemModelerTool::SteamPropertiesOutput SteamProperties::waterPropertiesPressureEntropy(const double pressure,
    ...
    if (entropy < specificEntropyLimit) {
        if (pressure > SteamSystemModelerTool::PRESSURE_Tp) {
            region13boundary =
                SteamProperties::waterPropertiesPressureTemperature(pressure, SteamSystemModelerTool::TEMPERATURE_Tp);
        }
    ...
    if ((pressure < SteamSystemModelerTool::PRESSURE_CRIT) && (entropy >= pressureSatProps.liquidSpecificEntropy) &&
        ...
        return {
            ...
            (pressureSatProps.gasSpecificEnthalpy - pressureSatProps.liquidSpecificEnthalpy) * quality +
                pressureSatProps.liquidSpecificEnthalpy,
            entropy};
    }
...
SteamSystemModelerTool::SteamPropertiesOutput SteamProperties::waterPropertiesPressureQuality(const double pressure,
