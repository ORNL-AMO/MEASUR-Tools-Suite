...
Turbine::Turbine(Solve solveFor, double inletPressure, SteamProperties::ThermodynamicQuantity inletQuantity,
    ...
    : solveFor(solveFor), inletPressure(inletPressure), isentropicEfficiency(isentropicEfficiency / 100),
      ...
      outletSteamPressure(outletSteamPressure), inletQuantity(inletQuantity), inletQuantityValue(inletQuantityValue),
...
Turbine::Turbine(const Solve solveFor, const double inletPressure,
    ...
    : solveFor(solveFor), inletPressure(inletPressure), generatorEfficiency(generatorEfficiency / 100),
      massFlowOrPowerOut(massFlowOrPowerOut), outletSteamPressure(outletSteamPressure), inletQuantity(inletQuantity),
      outletQuantity(outletQuantity), inletQuantityValue(inletQuantityValue), outletQuantityValue(outletQuantityValue),
      turbineProperty(turbineProperty) {
    calculate();
}
...
void Turbine::solveForOutletProperties() {
    ...
    outletProperties = SteamProperties(outletSteamPressure, SteamProperties::ThermodynamicQuantity::ENTROPY,
                                       inletProperties.specificEntropy)
                           .calculate();
    auto const idealOutletSpecificEnthalpy = outletProperties.specificEnthalpy;

    auto const outletSpecificEnthalpy =
        inletSpecificEnthalpy - isentropicEfficiency * (inletSpecificEnthalpy - idealOutletSpecificEnthalpy);

    outletProperties =
        SteamProperties(outletSteamPressure, SteamProperties::ThermodynamicQuantity::ENTHALPY, outletSpecificEnthalpy)
            .calculate();
...
void Turbine::solveForIsentropicEfficiency() {
    ...
    outletProperties = SteamProperties(outletSteamPressure, outletQuantity, outletQuantityValue).calculate();
...
std::ostream& operator<<(std::ostream& stream, const Turbine& turbine) {
    stream << "Turbine[" << "solveFor=" << static_cast<int>(turbine.solveFor)
           ...
           << ", outletQuantityValue=" << turbine.outletQuantityValue << ", inletProperties=" << turbine.inletProperties
           << ", outletProperties=" << turbine.outletProperties << ", inletEnergyFlow=" << turbine.inletEnergyFlow
           << ", outletEnergyFlow=" << turbine.outletEnergyFlow << ", energyOut=" << turbine.energyOut
           << ", powerOut=" << turbine.powerOut << ", massFlow=" << turbine.massFlow << "]";
    return stream;
}
