...
std::ostream& operator<<(std::ostream& stream, const PrvWithoutDesuperheating& prv) {
    stream << "PrvWithoutDesuperheating[" << "inletPressure=" << prv.inletPressure
           << ", inletMassFlow=" << prv.inletMassFlow << ", inletEnergyFlow=" << prv.inletEnergyFlow
           << ", quantityValue=" << prv.quantityValue << ", quantityType=" << static_cast<int>(prv.quantityType)
           << ", inletProperties=" << prv.inletProperties << ", outletPressure=" << prv.outletPressure
...
void PrvWithoutDesuperheating::calculateProperties() {
    ...
    outletProperties = SteamProperties(outletPressure, SteamProperties::ThermodynamicQuantity::ENTHALPY,
                                       inletProperties.specificEnthalpy)
                           .calculate();
...
PrvWithDesuperheating::PrvWithDesuperheating(const double                                 inletPressure,
    ...
    : PrvWithoutDesuperheating(inletPressure, quantityType, quantityValue, inletMassFlow, outletPressure),
      feedwaterPressure(feedwaterPressure), feedwaterQuantityValue(feedwaterQuantityValue),
      desuperheatingTemp(desuperheatingTemp), feedwaterQuantityType(feedwaterQuantityType) {
    calculateProperties();
}

std::ostream& operator<<(std::ostream& stream, const PrvWithDesuperheating& prv) {
    stream << "PrvWithDesuperheating[" << "inletPressure=" << prv.inletPressure
           << ", inletMassFlow=" << prv.inletMassFlow << ", inletEnergyFlow=" << prv.inletEnergyFlow
           << ", quantityValue=" << prv.quantityValue << ", quantityType=" << static_cast<int>(prv.quantityType)
           << ", inletProperties=" << prv.inletProperties << ", outletPressure=" << prv.outletPressure
           << ", outletProperties=" << prv.outletProperties
           ...
           << ", feedwaterProperties=" << prv.feedwaterProperties << ", inletEnergyFlow=" << prv.inletEnergyFlow
           << ", outletMassFlow=" << prv.outletMassFlow << ", outletEnergyFlow=" << prv.outletEnergyFlow
           << ", feedwaterMassFlow=" << prv.feedwaterMassFlow << ", feedwaterEnergyFlow=" << prv.feedwaterEnergyFlow
           << "]";
    return stream;
}
...
void PrvWithDesuperheating::calculateProperties() {
    ...
    outletProperties =
        SteamProperties(outletPressure, SteamProperties::ThermodynamicQuantity::TEMPERATURE, desuperheatingTemp)
            .calculate();
