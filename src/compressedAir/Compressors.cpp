...
CompressorsBase::OutputBlowOff
Compressors_Centrifugal_BlowOff::calculateFromVIPFMeasured_BlowOff(double V, double I, double PF, double blowPer) {
...
CompressorsBase::Output Compressors_Centrifugal_ModulationUnload::calculateFromPerC(double CPer) {
    ...
    if (CPer < kWPer_ul)
        ...
        PerkW = ((kWPer_max - kWPer_ul) / (CPer_max - CPer_ul)) * (CPer) + kWPer_max -
                ((kWPer_max - kWPer_ul) / (CPer_max - CPer_ul)) * CPer_max;
...
CompressorsBase::Output Compressors_Centrifugal_ModulationUnload::calculateFromVIPFMeasured(double V, double I,
                                                                                            double PF) {
...
double Compressors_LoadUnload::CurveFit(double value, bool capacityVPower) const {
    ...
    do {
        ...
        if (C_curve < C_ul) {
            ...
            const double kW_avg_bd = kW_nl + (((kW_ul - kW_maxmod) * exp(-t_bd / t_spc) + kW_maxmod) - kW_nl) *
                                                 ((1 - exp(-t_bd / t_bdc)) * (t_bdc / t_bd));
            ...
            if (t_bd == t_blowdown) {
                ...
                kW_min_bd =
                    kW_nl + ((((kW_ul - kW_maxmod) * exp(-t_bd / t_spc) + kW_maxmod) - kW_nl) * exp(-t_bd / t_bdc));
            }
            ...
            const double t_rpu =
                60 * C_storage *
                ((P_max - P_fl + (t_rl / 60) * P_atm * (C_curve / C_storage)) / (P_atm * (C_fl - C_curve)));
...
CompressorsBase::Output Compressors_LoadUnload::calculateFromPerkW(double PerkW) {
    ...
    if (CntrlType == ControlType::ModulationUnload || CntrlType == ControlType::VariableDisplacementUnload) {
        ...
        if (kW_avg >= kW_ul) {
            ...
            return Compressors_ModulationWOUnload(kW_fl, C_fl, kW_avg,
                                                  CntrlType == ControlType::VariableDisplacementUnload ? 2 : 1, false,
                                                  CompType, noLoadPowerFM, kW_max)
                .calculateFromPerkW(PerkW);
        }
    }
...
CompressorsBase::Output Compressors_LoadUnload::calculateFromPerC(double CPer) {
    ...
    if (CntrlType == ControlType::ModulationUnload || CntrlType == ControlType::VariableDisplacementUnload) {
        ...
        if (C_fl * CPer >= C_ul) {
            ...
            return Compressors_ModulationWOUnload(kW_fl, C_fl, kW_max,
                                                  CntrlType == ControlType::VariableDisplacementUnload ? 2 : 1, false,
                                                  CompType, noLoadPowerFM)
                .calculateFromPerC(CPer);
        }
    }
