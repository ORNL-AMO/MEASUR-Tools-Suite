...
void InsulatedPipeReduction::validateInput(InsulatedPipeInput input) {
    ...
    if (input.getPipeMaterialCoefficients().size() != 5) {
        throw "ERROR : Pipe - expected exactly 5 pipe material coefficients, received " +
            std::to_string(input.getPipeMaterialCoefficients().size());
    }
    if (input.getInsulationMaterialCoefficients().size() != 5) {
        throw "ERROR : Pipe - expected exactly 5 insulation material coefficients, received " +
            std::to_string(input.getInsulationMaterialCoefficients().size());
    }
}
...
InsulatedPipeOutput InsulatedPipeReduction::calculateInsulation(InsulatedPipeInput input) {
    ...
    // start iteration
    heatLength     = InsulatedPipeReduction::insulationRecursive(input, innerPipeDiameter, outerInsulationDiameter,
                                                                 surfaceTemperature, interfaceTemperature, 0, 0);
...
double InsulatedPipeReduction::insulationRecursive(InsulatedPipeInput input, double innerPipeDiameter,
                                                   double insulationOuterDiameter, double surfaceTemperature,
                                                   double interfaceTemperature, double heatLength, int i) {
    ...
    double insReynolds =
        calculateReynoldsNumber(insulationOuterDiameter, input.getWindVelocity(), airProperties.getKinViscosity());
    double insRayleigh      = calculateRayleighNumber(airProperties.getExpansionCoefficient(), surfaceTemperature,
                                                      input.getAmbientTemperature(), insulationOuterDiameter,
                                                      airProperties.getKinViscosity(), airProperties.getAlpha());
    double insHeatRadiation = calculateRadiativeHeatTransferCoefficient(input.getJacketEmissivity(), surfaceTemperature,
                                                                        input.getAmbientTemperature());
    ...
    double insConvectionCombined = calculateConvectiveHeatTransferCoefficient(
        insNusseltCombined, airProperties.getConductivity(), insulationOuterDiameter);
    ...
    double pipeResistance =
        calculateThermalResistance(insulationOuterDiameter, input.getPipeDiameter(), innerPipeDiameter, kPipe);
    ...
    double insulationResistance  = calculateThermalResistance(insulationOuterDiameter, insulationOuterDiameter,
                                                              input.getPipeDiameter(), kInsulation);
    ...
    if (std::fabs(heatLengthNew - heatLength) < 0.0001) {
        ...
        i = i + 1;
        return InsulatedPipeReduction::insulationRecursive(input, innerPipeDiameter, insulationOuterDiameter,
                                                           surfaceTemperature, interfaceTemperature, heatLengthNew, i);
    }
}
...
InsulatedPipeOutput InsulatedPipeReduction::calculateNoInsulation(InsulatedPipeInput input) {
    ...
    // start iteration
    heatLength     = InsulatedPipeReduction::noInsulationRecursive(input, innerPipeDiameter, insulationOuterDiameter,
                                                                   surfaceTemperature, interfaceTemperature, 0.0, 0);
    annualHeatLoss = heatLength * input.getPipeLength() * input.getOperatingHours() / input.getSystemEfficiency();
    return InsulatedPipeOutput(heatLength, annualHeatLoss);
}

double InsulatedPipeReduction::noInsulationRecursive(InsulatedPipeInput input, double innerPipeDiameter,
                                                     double insulationOuterDiameter, double surfaceTemperature,
                                                     double interfaceTemperature, double heatLength, int i) {
    double filmTemperature;
    filmTemperature = (surfaceTemperature + input.getAmbientTemperature()) / 2.0;
    ...
    // step 2: air film resistance
    double bareReynolds =
        calculateReynoldsNumber(input.getPipeDiameter(), input.getWindVelocity(), airProperties.getKinViscosity());
    double bareRayleigh = calculateRayleighNumber(airProperties.getExpansionCoefficient(), surfaceTemperature,
                                                  input.getAmbientTemperature(), input.getPipeDiameter(),
                                                  airProperties.getKinViscosity(), airProperties.getAlpha());
    double bareRadiativeHeatTransferCoefficient = calculateRadiativeHeatTransferCoefficient(
        input.getPipeEmissivity(), surfaceTemperature, input.getAmbientTemperature());
    ...
    double bareConvection      = calculateConvectiveHeatTransferCoefficient(
        bareNusseltCombined, airProperties.getConductivity(), input.getPipeDiameter());
    double bareHeatTransferAir = bareRadiativeHeatTransferCoefficient + bareConvection;

    // step 3: pipe resistance
    double kPipe = this->propertyFit(input.getPipeMaterialCoefficients(), input.getPipeTemperature());
    double pipeResistance =
        calculateThermalResistance(input.getPipeDiameter(), input.getPipeDiameter(), innerPipeDiameter, kPipe);
    ...
    if (std::fabs(heatLengthNew - heatLength) < 0.0001) {
        ...
        return InsulatedPipeReduction::noInsulationRecursive(input, innerPipeDiameter, insulationOuterDiameter,
                                                             surfaceTemperature, interfaceTemperature, heatLengthNew,
                                                             i++);
    }
}
...
InsulationFluidProperties InsulatedPipeReduction::calculateAirProperties(double temp) {
    ...
    return InsulationFluidProperties(airConductivity, airViscosity, airPrandtl, airExpansionCoefficient, airDensity,
                                     airKinViscosity, airSpecificHeat, airAlpha, 0, 0);
}

double InsulatedPipeReduction::calculateAirProperty(int property, double temp) {
    std::vector<double> coefficients = {InsulatedPipeReduction::lookupAirPropertyCoefficient(property, 0),
...
const double* InsulatedPipeReduction::lookupAirProperty(int property) { return this->_airPropertiesArray[property]; }

double InsulatedPipeReduction::calculateThermalResistance(double diameterA, double diameterB, double diameterC,
                                                          double thermalConductivity) {
    return diameterA * std::log(diameterB / diameterC) / (2.0 * thermalConductivity);
}

double InsulatedPipeReduction::calculateReynoldsNumber(double diameter, double windVelocity,
                                                       double kinematicViscosity) {
    return diameter * windVelocity / kinematicViscosity;
}

double InsulatedPipeReduction::calculateRayleighNumber(double expCoefficient, double surfaceTemperature,
                                                       double ambientTemperature, double diameter,
                                                       double kinematicViscosity, double alpha) {
    double g                 = 9.81;
...
double InsulatedPipeReduction::calculateRadiativeHeatTransferCoefficient(double emissivity, double surfaceTemperature,
                                                                         double ambientTemperature) {
    double c = 0.00000005670373;
    return c * emissivity * (std::pow(surfaceTemperature, 4) - std::pow(ambientTemperature, 4)) /
           (surfaceTemperature - ambientTemperature);
}

double InsulatedPipeReduction::calculateConvectiveHeatTransferCoefficient(double nusselt, double airConductivity,
                                                                          double diameter) {
