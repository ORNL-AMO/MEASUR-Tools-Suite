...
WaterReduction::Output WaterReduction::calculate() {
    ...
    for (auto& waterReductionInput : waterReductionInputVec) {
        ...
        if (waterReductionInput.getMeasurementMethod() == 0) {
            ...
            tmpWaterUse =
                ((volumeMeterMethodData.getFinalMeterReading() - volumeMeterMethodData.getInitialMeterReading()) /
            ...
            tmpWaterUse =
                (bucketMethodData.getBucketVolume() / (bucketMethodData.getBucketFillTime() * (1.0 / 3600.0))) *
                waterReductionInput.getOperatingHours();
            tmpWaterCost = waterReductionInput.getWaterCost() * tmpWaterUse;
        }
        else {
...
// Metered Flow Data
void MeteredFlowMethodData::setMeterReading(double meterReading) { this->meterReading = meterReading; }
...
void VolumeMeterMethodData::setElapsedTime(double elapsedTime) { this->elapsedTime = elapsedTime; }

// Bucket Method Data
void BucketMethodData::setBucketVolume(double bucketVolume) { this->bucketVolume = bucketVolume; }

void BucketMethodData::setBucketFillTime(double bucketFillTime) { this->bucketFillTime = bucketFillTime; }

// Water Other Method Data
void WaterOtherMethodData::setConsumption(double consumption) { this->consumption = consumption; }