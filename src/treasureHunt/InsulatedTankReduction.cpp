...
InsulatedTankOutput InsulatedTankReduction::calculateInsulation(InsulatedTankInput input)
{
    ...
    InsulationFluidProperties airProperties = InsulatedTankReduction::calculateAirProperties(input.getAmbientTemperature());
    ...
    airRayleigh = (32.174 * thermalExpansionCoefficient * std::fabs(input.getSurfaceTemperature() - input.getAmbientTemperature()) * std::pow(input.getTankDiameter(), 3)) / (airProperties.getKinViscosity() * thermalDiffusivity);
    ...
    insulationRadiativeHeatLoss = 0.1713e-8 * input.getJacketEmissivity() * (std::pow(input.getSurfaceTemperature(), 4) - std::pow(input.getAmbientTemperature(), 4)) / 1e5;
...
InsulationFluidProperties InsulatedTankReduction::calculateAirProperties(double temp)
{
    ...
    double reynolds = 0;
    double rayleigh = 0;
    return InsulationFluidProperties(airConductivity, viscosity, prandtl, expansionCoefficient, airDensity, airKinViscosity, airSpecificHeat, alpha, reynolds, rayleigh);
}

double InsulatedTankReduction::calculateAirProperty(int property, double temp)
{
    std::vector<double> coefficients = {
            InsulatedTankReduction::lookupAirPropertyCoefficient(property, 0),
