...
SteamReduction::Output SteamReduction::calculate() {
    ...
    for (auto& steamReductionInput : steamReductionInputVec) {
        ...
        double steamEnthalpy =
            SteamProperties(steamReductionInput.getPressure(), steamReductionInput.getSteamVariableOption(),
                            steamReductionInput.getSteamVariable())
                .calculate()
                .specificEnthalpy;
        double feedEnthalpy = SteamProperties(0.101325, SteamProperties::ThermodynamicQuantity::TEMPERATURE,
                                              steamReductionInput.getFeedWaterTemperature())
                                  .calculate()
                                  .specificEnthalpy;
        ...
        if (steamReductionInput.getMeasurementMethod() == 0) {
            ...
            tmpSteamUse = flowMeterMethodData.getFlowRate() * steamReductionInput.getHoursPerYear() *
                          (steamReductionInput.getUnits() / steamReductionInput.getSystemEfficiency());
        ...
        // air mass flow
            ...
            double heatFlowRate =
                (0.072381 * airFlowRate *
                 (massFlowMethodData.getOutletTemperature() - massFlowMethodData.getInletTemperature()));
            tmpEnergyUse = (steamReductionInput.getUnits() * (heatFlowRate)*steamReductionInput.getHoursPerYear()) /
                           (steamReductionInput.getSystemEfficiency() * steamReductionInput.getBoilerEfficiency());
