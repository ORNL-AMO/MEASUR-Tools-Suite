    ...
    for (auto& electricityReductionInput : electricityReductionInputVec) {
        ...
        if (electricityReductionInput.getMeasurementMethod() == 0) {
            ...
            if (multimeterData.getNumberOfPhases() == 1) {
                tmpTotalPower = multimeterData.getSupplyVoltage() * multimeterData.getAverageCurrent() *
                                multimeterData.getPowerFactor() * 0.001 * electricityReductionInput.getUnits();
            }
            else {
                tmpTotalPower = multimeterData.getSupplyVoltage() * multimeterData.getAverageCurrent() *
                                multimeterData.getPowerFactor() * (sqrt(3) / 1000) *
                                electricityReductionInput.getUnits();
            }
            ...
            tmpTotalPower               = nameplateData.getRatedMotorPower() * nameplateData.getLoadFactor() *
                            pow((nameplateData.getOperationalFrequency() / nameplateData.getLineFrequency()), 2.5) *
                            (1 / nameplateData.getMotorAndDriveEfficiency());
            tmpEnergyUse =
                tmpTotalPower * electricityReductionInput.getOperatingHours() * electricityReductionInput.getUnits();
            tmpEnergyCost = tmpEnergyUse * electricityReductionInput.getElectricityCost();
            energyUse     = energyUse + tmpEnergyUse;
            energyCost    = energyCost + tmpEnergyCost;
...
void ElectricityReduction::setElectricityReductionInputVec(
    std::vector<ElectricityReductionInput>& electricityReductionInput) {
    this->electricityReductionInputVec = std::move(electricityReductionInput);
}

// MultimeterData object setters
void MultimeterData::setNumberOfPhases(const int numberOfPhases) { this->numberOfPhases = numberOfPhases; }

void MultimeterData::setSupplyVoltage(const double supplyVoltage) { this->supplyVoltage = supplyVoltage; }

void MultimeterData::setAverageCurrent(const double averageCurrent) { this->averageCurrent = averageCurrent; }

void MultimeterData::setPowerFactor(const double powerFactor) { this->powerFactor = powerFactor; }

// NameplateData object setters
void NameplateData::setRatedMotorPower(const double ratedMotorPower) { this->ratedMotorPower = ratedMotorPower; }
...
void NameplateData::setLineFrequency(const double lineFrequency) { this->lineFrequency = lineFrequency; }
...
void NameplateData::setLoadFactor(const double loadFactor) { this->loadFactor = loadFactor; }

// PowerMeterData object setters
void PowerMeterData::setPower(const double power) { this->power = power; }

// Other Method Data
void OtherMethodData::setEnergy(const double energy) { this->energy = energy; }