...
CompressedAirReduction::Output CompressedAirReduction::calculate() {
    ...
    for (auto& compressedAirReductionInput : compressedAirReductionInputVec) {
        double tmpEnergyUse = 0, tmpEnergyCost = 0, tmpFlowRate = 0, tmpSingleNozzleFlowRate = 0,
               tmpTotalConsumption = 0;
        ...
        if (compressedAirReductionInput.getMeasurementMethod() == 0) {
            ...
            tmpTotalConsumption                                  = flowMeterMethodData.getMeterReading() * 60.0 *
                                  compressedAirReductionInput.getHoursPerYear() *
                                  compressedAirReductionInput.getUnits();
        ...
        if (compressedAirReductionInput.getUtilityType() == 0) {
            ...
            CompressorElectricityData compressorElectricityData =
                compressedAirReductionInput.getCompressorElectricityData();
    ...
    return CompressedAirReduction::Output(totalEnergyUse, totalEnergyCost, totalFlowRate, singleNozzleFlowRate,
                                          totalConsumption);
}
