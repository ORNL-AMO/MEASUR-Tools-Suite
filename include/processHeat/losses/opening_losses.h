 ...
 * Used to calculateThermalResistance the heat loss caused by heat transfer from the openings on equipment to the
 * surrounding. Only considers thermal radiation losses due to exposure of furnace to the surroundings.
 */
class OpeningLosses {
  public:
    /**
     ...
     */
    OpeningLosses(double emissivity, double length, double width, double thickness, double ratio,
                  double ambientTemperature, double insideTemperature, double percentTimeOpen, double viewFactor)
        : emissivity(emissivity), length(length), width(width), thickness(thickness), ratio(ratio),
          ambientTemperature(ambientTemperature), insideTemperature(insideTemperature),
          percentTimeOpen(percentTimeOpen), viewFactor(viewFactor), openingShape(OpeningShape::RECTANGULAR) {}
     ...
     * @param viewFactor double, view factor - unitless
     */
    OpeningLosses(double emissivity, double diameter, double thickness, double ratio, double ambientTemperature,
                  double insideTemperature, double percentTimeOpen, double viewFactor)
        : emissivity(emissivity), diameter(diameter), thickness(thickness), ratio(ratio),
          ambientTemperature(ambientTemperature), insideTemperature(insideTemperature),
          percentTimeOpen(percentTimeOpen), viewFactor(viewFactor), openingShape(OpeningShape::CIRCULAR) {}
     ...
     * @param ambientTemperature double, ambient temperature in °F
     */
    void setAmbientTemperature(double ambientTemperature) { this->ambientTemperature = ambientTemperature; }

    /**
     * Gets the inside temperature
     * @return double, inside temperature in °F
     */
    double getInsideTemperature() const { return insideTemperature; }

    /**
     * Sets the inside temperature
     * @param insideTemperature double, inside temperature in °F
     */
    void setInsideTemperature(double insideTemperature) { this->insideTemperature = insideTemperature; }

    /**
     * Gets the percentage of time open
     * @return double, amount of time open as %
     */
    double getPercentTimeOpen() const { return percentTimeOpen; }

    /**
     * Sets the percentage of time open
     * @param percentTimeOpen double, amount of time open as a %
     */
    void setPercentTimeOpen(double percentTimeOpen) { this->percentTimeOpen = percentTimeOpen; }

    /**
     * Gets the view factor
     * @return double, view factor - unitless
     */
    double getViewFactor() const { return viewFactor; }

    /**
     * Sets the view factor manually, can also be calculated via one of the calculateViewFactor methods
     * @param viewFactor double, view factor - unitless
     */
    void setViewFactor(double viewFactor) { this->viewFactor = viewFactor; }

    /**
     * Gets the opening shape
     * @return OpeningShape, classification of shape of opening
     */
    OpeningShape getOpeningShape() const { return openingShape; }
    ...
    const std::array<const std::function<double(const double thicknessRatio)>, 8> viewFactorEquations = {{
        ...
        [](const double tr) {
            return (26.9999999903567 + 64.5666666806646 * tr - 29.9166666745008 * std::pow(tr, 2) +
                    ...
                    3.74999999085373E-02 * std::pow(tr, 5)) /
                   100;
        }, // case 8
    }};
};
