 ...
 * Used to calculateThermalResistance the heat loss caused by gases leaving the furnace via openings other then the
 * flue. This calculator should only be used if the furnace is operating at a positive pressure.
 */
class LeakageLosses {
  public:
    /**
     ...
     */
    LeakageLosses(double draftPressure, double openingArea, double leakageGasTemperature, double ambientTemperature,
                  double coefficient, double specificGravity, double correctionFactor)
        : draftPressure(draftPressure), openingArea(openingArea), leakageGasTemperature(leakageGasTemperature),
          ambientTemperature(ambientTemperature), coefficient(coefficient), specificGravity(specificGravity),
     ...
     */
    double getDraftPressure() const { return draftPressure; }

    /**
     * Sets the furnace draft pressure
     * @param draftPressure double, draft pressure in inch W.C.
     */
    void setDraftPressure(double draftPressure) { this->draftPressure = draftPressure; }

    /**
     * Gets the opening area
     * @return double, opening area in ft^2
     */
    double getOpeningArea() const { return openingArea; }

    /**
     * Sets the opening area
     * @param openingArea double, opening area in ft^2
     */
    void setOpeningArea(double openingArea) { this->openingArea = openingArea; }

    /**
     * Gets the temperature of gases leaking from furnace
     * @return double, leakeage gas temperature in °F
     */
    double getLeakageGasTemperature() const { return leakageGasTemperature; }

    /**
     * Sets the temperature of gases leaking from furnace
     * @param leakageGasTemperature double, leakage gas temperature in °F
     */
    void setLeakageGasTemperature(double leakageGasTemperature) { this->leakageGasTemperature = leakageGasTemperature; }
     ...
     * @param coefficient double, coefficient - unitless
     */
    void setCoefficient(double coefficient) { this->coefficient = coefficient; }
