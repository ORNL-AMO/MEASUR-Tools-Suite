...
class WaterHeatingUsingFlue {
  public:
    ...
    struct Output {
        Output(double flowFlueGas, double effBoiler, double enthalpySteam, double enthalpyFW, double flowSteam,
            ...
            : flowFlueGas(flowFlueGas), effBoiler(effBoiler), enthalpySteam(enthalpySteam), enthalpyFW(enthalpyFW),
              flowSteam(flowSteam), flowFW(flowFW), specheatFG(specheatFG), heatCapacityFG(heatCapacityFG),
              specheatFW(specheatFW), heatCapacityFW(heatCapacityFW), heatCapacityMin(heatCapacityMin),
              ratingHeatRecFW(ratingHeatRecFW), tempFlueGasOut(tempFlueGasOut), tempFWOut(tempFWOut),
              energySavingsBoiler(energySavingsBoiler), costSavingsBoiler(costSavingsBoiler) {}

        Output()           = default;
        double flowFlueGas = 0, effBoiler = 0, enthalpySteam = 0, enthalpyFW = 0, flowSteam = 0, flowFW = 0,
               specheatFG = 0, heatCapacityFG = 0, specheatFW = 0, heatCapacityFW = 0, heatCapacityMin = 0,
               ratingHeatRecFW = 0, tempFlueGasOut = 0, tempFWOut = 0, energySavingsBoiler = 0, costSavingsBoiler = 0;
    };
    ...
    Output calculate(GasCompositions gasCompositions, const double tempFlueGasF, const double percO2,
                     ...
                     const double hhvFuel, const SteamCondition condSteam = Superheated, const double fuelTempF = 60);
