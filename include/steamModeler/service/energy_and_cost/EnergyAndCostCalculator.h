...
class EnergyAndCostCalculator {
  public:
    ...
    double calcPowerGenerated(
        const HighPressureHeaderCalculationsDomain&                    highPressureHeaderCalculationsDomain,
        const std::shared_ptr<MediumPressureHeaderCalculationsDomain>& mediumPressureHeaderCalculationsDomain) const;

    double addPowerOutToPowerGenerated(const std::string& name, const std::shared_ptr<Turbine>& turbine,
                                       const double powerGenerated) const;

    double calcPowerImport(const bool isBaselineCalc, const double sitePowerImportInput,
    ...
    double calcPowerGenerationCost(const double sitePowerImport, const double electricityCosts,
                                   const double operatingHoursPerYear) const;

    double calcBoilerFuelCost(const double fuelEnergyInput, const double operatingHoursPerYearInput,
    ...
    double calcTotalOperatingCost(const double powerGenerationCost, const double boilerFuelCost,
