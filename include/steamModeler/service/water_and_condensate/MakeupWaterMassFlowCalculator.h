...
class MakeupWaterMassFlowCalculator {
  ...
  private:
    ...
    double calcInletHeaderFlow(const SteamSystemModelerTool::FluidProperties& highPressureHeaderOutput,
                               const HeaderWithHighestPressure&               highPressureHeaderInput) const;

    double calcInletHeaderFlow(const SteamSystemModelerTool::FluidProperties&   lowPressureHeaderOutput,
                               const std::shared_ptr<HeaderNotHighestPressure>& lowPressureHeaderInput) const;

    double calcMakeupWaterMassFlow(
    ...
    double addPrvFeedwaterMassFlowToMakeupWaterMassFlow(const std::shared_ptr<PrvWithoutDesuperheating>& prv,
                                                        double makeupWaterMassFlow) const;

    double getFeedwaterMassFlow(const std::shared_ptr<PrvWithoutDesuperheating>& prv) const;

    double calcMakeupWaterEnergyFlow(double                                               massFlow,
