...
class ProcessSteamUsageModeler {
  ...
  private:
    ProcessSteamUsage calc(const HeaderWithHighestPressure&            highPressureHeaderInput,
                           const HighPressureHeaderCalculationsDomain& highPressureHeaderCalculationsDomain) const;

    ProcessSteamUsage calc(const std::shared_ptr<HeaderNotHighestPressure>& headerInput,
