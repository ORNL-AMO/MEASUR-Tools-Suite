...
class SteamMassFlowMethodData {
  public:
     ...
     */
    SteamMassFlowMethodData(const bool isNameplate, const SteamMassFlowMeasuredData massFlowMeasuredData,
                            const SteamMassFlowNameplateData massFlowNameplateData, const double inletTemperature,
                            const double outletTemperature)
        : isNameplate(isNameplate), massFlowMeasuredData(massFlowMeasuredData),
          massFlowNameplateData(massFlowNameplateData), inletTemperature(inletTemperature),
...
class SteamReductionInput {
  public:
     ...
     * @param steamVariable double, Thermodynamic Property used for calculation- Temperature (K), Enthalpy (kJ/kg),
     * Entropy (kJ/kg-K), or Quality (unit less)
     * @param feedWaterTemperature double, units K
     */
    SteamReductionInput(const int hoursPerYear, const int utilityType, const double utilityCost,
                        const int measurementMethod, const double systemEfficiency, const double pressure,
                        ...
                        const SteamOffsheetMethodData offsheetMethodData, const int units,
                        const double boilerEfficiency, const SteamProperties::ThermodynamicQuantity steamVariableOption,
                        const double steamVariable, const double feedWaterTemperature)
        : hoursPerYear(hoursPerYear), utilityType(utilityType), utilityCost(utilityCost),
          measurementMethod(measurementMethod), systemEfficiency(systemEfficiency), pressure(pressure),
          flowMeterMethodData(flowMeterMethodData), airMassFlowMethodData(airMassFlowMethodData),
          waterMassFlowMethodData(waterMassFlowMethodData), offsheetMethodData(offsheetMethodData), units(units),
          boilerEfficiency(boilerEfficiency), steamVariableOption(steamVariableOption), steamVariable(steamVariable),
          feedWaterTemperature(feedWaterTemperature) {}
...
class SteamReduction {
  public:
     ...
     */
    explicit SteamReduction(std::vector<SteamReductionInput> steamReductionInputVec)
        : steamReductionInputVec(std::move(steamReductionInputVec)) {}
