...
class OperatingCost {
  public:
     ...
     * @param motorBhp double, Brake Horse Power is the input power required at the compressor input shaft for a
     * specific speed, capacity and pressure condition. It is generally shown on the compressor panel. - bhp
    ...
    struct Output {
        Output(const double runTimeUnloaded, const double costForLoaded, const double costForUnloaded,
            ...
            : runTimeUnloaded(runTimeUnloaded), costForLoaded(costForLoaded), costForUnloaded(costForUnloaded),
    ...
    Output calculate() {
        ...
        auto const costForLoaded =
            (motorBhp * 0.746 * annualOperatingHours * costOfElectricity * (runTimeLoaded / 100)) /
            (efficiencyLoaded / 100);
        auto const costForUnloaded = (motorBhp * 0.746 * annualOperatingHours * costOfElectricity *
                                      (bhpUnloaded / 100) * (runTimeUnloaded / 100)) /
