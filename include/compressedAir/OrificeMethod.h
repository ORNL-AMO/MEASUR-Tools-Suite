...
class OrificeMethod {
  public:
    struct Output {
        Output(const double standardDensity, const double sonicDensity, const double leakVelocity,
               const double leakRateLBMmin, const double leakRateScfm, const double leakRateEstimate,
        ...
        const double standardDensity, sonicDensity, leakVelocity, leakRateLBMmin, leakRateScfm, leakRateEstimate,
            annualConsumption;
    };

    /**
     * Constructor for OrificeMethod - The orifice method estimates the air loss by using the pressure and diameter of
     * the orifice
     ...
     * @param dischargeCoef double, discharge coefficient used to capture the effect of the shape of the outlet on air
     * loss
     ...
     */
    OrificeMethod(const double operatingTime, const double airTemp, const double atmPressure,
                  const double dischargeCoef, const double diameter, const double supplyPressure, const int numOrifices)
        : operatingTime(operatingTime), airTemp(airTemp), atmPressure(atmPressure), dischargeCoef(dischargeCoef),
          diameter(diameter), supplyPressure(supplyPressure), numOrifices(numOrifices) {}

    /**
     * @return OrificeMethod::Output, standard density, sonic density, leak velocity, leak rate LBMmin, leak rate Scfm,
     * leak rate estimate, annual consumption
     */
    Output calculate() {
        ...
        OrificeMethod::Output output(standardDensity, sonicDensity, leakVelocity, leakRateLBMmin, leakRateScfm,
