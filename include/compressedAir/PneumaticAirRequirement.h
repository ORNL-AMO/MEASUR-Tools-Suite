...
class PneumaticAirRequirement {
  public:
     ...
     * This calculator computes the quantity of air required by a specific single acting or a double acting piston
     * cylinder compressor. The design specs of the compressor are entered in the calculator and the quantity of air
     * needed is generated.
     ...
     * @param pistonRodDiameter double, Diameter of the piston rod (required only in case of a double acting cylinder) -
     * inches
     * @param airPressure double, Pressure of the air coming out of the cylinder - psi
     * @param cyclesPerMin double, Number of cycles (1 cycle is a combination of 2 strokes of a piston) by the
     * crankshaft in a minute - strokes
     */
    ...
    class Output {
      public:
         ...
         * @param airRequirementPneumaticCylinder const double, Total cubic feet/min of air requirement for a Pneumatic
         * cylinder - scfm
         */
        Output(const double volumeAirIntakePiston, const double compressionRatio,
               const double airRequirementPneumaticCylinder)
            : volumeAirIntakePiston(volumeAirIntakePiston), compressionRatio(compressionRatio),
     ...
     */
    Output calculate() {
        auto const volumeAirIntakeSingle =
            (0.785 * std::pow(cylinderDiameter, 2) * cylinderStroke * cyclesPerMin) / 1728;
        ...
        auto const volumeAirIntakeDouble = (2 * 1728 * volumeAirIntakeSingle -
                                            (0.785 * std::pow(pistonRodDiameter, 2) * cylinderStroke * cyclesPerMin)) /
                                           1728;
