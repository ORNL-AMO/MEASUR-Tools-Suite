...
class AirVelocity {
  public:
     ...
     * @param airFlow double, Volumetric flow rate of air in the compressor system, referenced to the compressor inlet
     * conditions - scfm
    ...
    PipeData calculate() {
        auto const compressedAirVelocity = [this](const double traverseArea) {
            return (airFlow * atmosphericPressure / (pipePressure + atmosphericPressure)) * (144 / traverseArea) *
                   (1.0 / 60);
        };
